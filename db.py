...
class SimpleDB:
    ...
    def create_table(self, table_name: str):
        """Create a new table using B+ tree for storage"""
    ...
    def read(self, table_name: str, key: Any):
        ...
        if table_name not in self.tables:
            return f"Table '{table_name}' does not exist"
        return self.tables[table_name].read(key)
    ...
    def delete(self, table_name: str, key: Any):
        ...
        if table_name not in self.tables:
            return f"Table '{table_name}' does not exist"
        if self.tables[table_name].delete(key):
    ...
    def load_db(self):
        ...
        for file_name in os.listdir(self.db_dir):
            if file_name.endswith('.db'):
                table_name = file_name[:-3]  # Remove .db extension
...
# Example usage
def demo_db():
    # First, let's create and populate the database
    print("\n=== Creating new database and adding records ===")
    db = SimpleDB("mydb")
    ...
    # Insert some records
    print(db.insert("users", 1, {"name": "John Doe", "age": 30}))
    print(db.insert("users", 2, {"name": "Jane Smith", "age": 25}))
    print(db.insert("users", 3, {"name": "Bob Jones", "age": 35}))
    
    # Update a record
    print(db.update("users", 1, {"name": "John Doe", "age": 31}))
    
    # Delete a record
    print(db.delete("users", 2))
    
    # Save the database
    print("Saving database to disk...")
    db.save_db()
    
    # Now let's demonstrate reading from the saved file
    print("\n=== Reading database from disk ===")
    new_db = SimpleDB("mydb")  # This will automatically load the saved data
    
    # Let's verify the data persisted by checking each record
    tree = new_db.tables.get("users")
    if tree:
        print("\nStored Records:")
        node = tree.root
